...
チーム情報の更新時、AIアシスタントは以下の形式で結果を通知します：

**✅ 更新成功の場合:**
- 公式サイトからチーム情報が正常に取得できた
- キャッチコピー（OB・実績・特徴など）を生成できた
- URLとチーム名が一致している

```
✅ 大阪柏原ボーイズ 更新完了！
- 公式サイト: https://example.com/
- キャッチコピー: プロOB輩出。専用グラウンド完備。
```
...
| 状況 | 通知内容 |
|------|----------|
| 情報が取得できない | サイトがアクセス不可、文字化け、メンテナンス中など |
| チーム名とURLが不一致 | 提供されたURLが別チームのサイト |
| チームが廃部 | 公式サイトに廃部・解散の記載がある |
| 地域情報のみ | OB・実績・特徴が見つからず、地域しか記載できない場合 |

```
❌ ○○ボーイズ - 更新失敗
- 理由: サイトがアクセス不可（メンテナンス中の可能性）
- 対応: URLのみ登録。後日再確認をお勧めします。

❌ △△リトルシニア - 情報不足
- 理由: 公式サイトに特徴的な情報がなく、地域情報のみ
- 登録内容: 「○○市を拠点に活動する硬式野球チーム。」
- 対応: 後日、Instagram等で追加情報があれば更新可能
```

**⚠️ 要確認の場合:**
- URLは有効だが、チーム名の表記が微妙に異なる場合
- 複数チームが同一サイトを共有している場合
...
女子専用チーム・女子部門があるチームには以下のルールで対応します：

**女子タグの付与基準:**

| チーム種別 | タグ | 例 |
|------------|------|-----|
| 女子専用チーム | `feature` に「女子チーム」を追加 | LSレディースリトルシニア、サザンガールズリトルシニア |
| 女子選手歓迎チーム | `feature` に「女子選手歓迎」を追加 | 横浜栄リトルシニア（男女問わず募集） |
| 女子部門併設 | キャッチコピーに記載 | 厚木リトルシニア（女子野球普及活動） |
...
```typescript
{
  ...
  prefecture: ['神奈川県'],
  league: ['シニア'],
  catchcopy: '南関東女子選抜チーム。強豪高校への進学実績多数。',
  feature1: '女子チーム',
  feature2: '選抜チーム',
  feature3: '強豪校進学',
}
